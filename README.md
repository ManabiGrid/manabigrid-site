...
`device_matrix_check.py`はローカル専用サーバーを自動起動し、小型320pxスマホから大型412pxスマホ、横向きスマホ、600／768／820pxタブレット、1024×768px横向きタブレット、390pxで文字200%相当までの10条件を同じ14代表ページへ適用します。ページ全体の横はみ出し、局所スクロール、資料あり／0件の単元ページ、検索・進捗・404などの操作、主要導線、ダーク配色、印刷への復帰を検査します。文字200%はCSS文字寸法を拡大する回帰proxyであり、実機OSの文字拡大機能そのものを完全再現するものではありません。
...
- 教材の`n/d`型の分数表記は、比、単位、URL、読み仮名等との機械的な区別ができないため、サイト側で一括置換しません。分数であることを正本側で明示した箇所だけ、既存MathML変換のような固定対象に追加します。

...
- 共通ヘッダーの「表示」で`自動／明るい／暗い`を選べます。既定の`自動`は端末設定へ従い、明示選択だけをこの端末のブラウザへ保存します。検索語、閲覧履歴、学習進捗は保存しません。
- 図の「図を大きく見る」は同じページ内の拡大表示を開きます。閉じるボタン、背景、Escキーで元の図とスクロール位置へ戻れます。JavaScriptが使えない場合も、新しいタブで元SVGを開くfallbackを残します。
- 横長の図と表は、実際に画面幅を超える時だけ横へスクロールできます。キーボード操作と読み上げ名、操作ヒントも実幅の判定に合わせて付けます。
- 全ページ末に「ページの先頭へ」と、ホームまたは教材一覧へ戻る導線を置きます。学習中の本文を隠さないよう、画面へ固定はしません。
...
公開ページは`index,follow`、ページごとのcanonical、重複しないtitleとdescription、`sitemap.xml`、OG情報を生成します。サイト内の全公開ページはトップからたどれる静的リンクを持ち、意図的な検索除外は行っていません。

ただし、Googleの掲載と掲載時期はサイト側だけでは保証できません。新規サイト公開後はGoogle Search ConsoleのURL-prefix propertyへ`https://manabigrid.github.io/manabigrid-site/`を登録し、`sitemap.xml`を送信する運用を推奨します。これはGoogleアカウントと所有権確認を伴う外部操作なので、ローカルbuildや日次更新runnerには組み込みません。
...
端末マトリクスは、11条件それぞれのブラウザレポートと、検査runner・ブラウザ検査器・CSS・生成器・端末契約のSHA-256を`review/browser/device-matrix-report.json`へ記録します。320pxと390pxの文字200%条件では適用前後のroot/body文字サイズも実測し、単に「倍率を指定した」だけの成功扱いをしません。
...
`device_matrix_check.py`はローカル専用サーバーを自動起動し、小型320pxスマホから大型412pxスマホ、横向きスマホ、600／768／820pxタブレット、1024×768px横向きタブレット、320px／390pxで文字200%相当までの11条件を同じ16代表ページへ適用します。ページ全体の横はみ出し、局所スクロール、資料あり／0件の単元ページ、検索・進捗・404などの操作、主要導線、ダーク配色、A4相当幅での印刷復帰に加え、固定3ページのインラインMathMLを検査します。文字200%で数式だけが横長になる場合は、フォーカス後に右矢印キーで実際に`scrollLeft`が動くことまで確認します。文字200%はCSS文字寸法を拡大する回帰proxyであり、実機OSの文字拡大機能そのものを完全再現するものではありません。
...
- `static/theme.js`: OS設定を既定とする3状態テーマ切替（外部送信なし）
...
- `site.config.json`: 公開URL、対象リポジトリ、OG画像、Search Console所有権確認値の単一設定
...
- 表示数式の静的MathML試作は、正本のIssue #21裁定（通常Markdown表記へ変更・2026-08-29）に伴い引退しました。表示数式は現在正本に存在しません（出現した場合は検査が違反として検出します）。
- これとは別に、正本を変えず、固定した代表3ページ・4式・計5箇所だけをインラインMathMLで試作しています。変数`x`と乗算記号`×`、正負記号、上下型分数を組版し、元表記を`annotation`として残します。
- 教材の`n/d`型や記号列は、比、単位、URL、日付、正誤記号等との機械的な区別ができないため、サイト側で一括置換しません。変換対象はsource path・元文字列・出現数を完全一致で固定し、未承認ページや件数ずれでは変換せずビルドを停止します。
- 日本語本文は禁則処理と対応ブラウザの文節改行を使いますが、文章の意味や段落境界を変える自動`br`挿入はしません。長い段落自体の分割は正本側の編集事項です。
- 中1数学「一次方程式」`lesson_07.md`の「みかんは（**10−x）個**」は正本由来です。括弧は全角ですが、開始括弧だけ強調範囲外のため太さが揃いません。サイト側で教材本文を改変せず、正本で「みかんは**（10−x）個**」へ直す候補として扱います。
- project site配下の`robots.txt`は生成しますが、origin直下ではないため検索エンジンが必ず参照するとは限りません。canonicalとsitemapを主なURL手掛かりにします。
- Search Consoleの所有権確認metaはトップページだけへ生成し、再ビルド後も維持します。確認値は公開情報であり、解析タグ、Cookie、閲覧追跡は追加しません。
